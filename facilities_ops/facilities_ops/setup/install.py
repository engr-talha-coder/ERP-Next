...
def after_install():
    ...
    try:
        ...
        frappe.logger().warning(f"facilities_ops: skipping departments — {e}")
    try:
        create_item_groups()
    except Exception as e:
        frappe.logger().warning(f"facilities_ops: skipping item groups — {e}")
    try:
        create_uom()
    except Exception as e:
        frappe.logger().warning(f"facilities_ops: skipping UOMs — {e}")
    frappe.db.commit()
...
def create_departments():
    ...
    if not company:
        frappe.logger().warning(
            "facilities_ops: No Company found; skipping Department creation. "
            "Re-run after completing ERPNext setup wizard."
        )
        return
...
def create_item_groups():
    if not frappe.db.exists("Item Group", "All Item Groups"):
        frappe.logger().warning(
            "facilities_ops: 'All Item Groups' not found; skipping Item Group creation."
        )
        return
